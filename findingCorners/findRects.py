      ...
      if(checkingRow[0].value != None ):
         if(currentH - checkingRow[2].value == currentW - checkingRow[3].value):
            if(currentX - checkingRow[0].value < 10 and currentY - checkingRow[1].value < 10):
               ws.delete_rows(checkingRow[0].row)

# Create coordinates
coordinatesWS = wb.create_sheet()
coordinatesWS.title = 'Coordinates'
coordinatesWS.append(['Top left (X,Y)', 'Top right', 'Bottom left', 'Bottom right'])
...
for row in coordinatesWS.iter_rows(min_row=2, max_row=None, min_col=None, max_col=None, values_only=False):
   for cell in row:
